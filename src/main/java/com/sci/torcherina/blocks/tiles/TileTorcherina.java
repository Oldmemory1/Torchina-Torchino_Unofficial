package com.sci.torcherina.blocks.tiles;

import java.util.Random;

import com.sci.torcherina.Torcherina;
import com.sci.torcherina.TorcherinaRegistry;
...
public class TileTorcherina extends TileEntity implements ITickable
{
    ...
    private void tickBlock(BlockPos pos)
    {
        ...
        if(block == null || block instanceof BlockFluidBase || TorcherinaRegistry.isBlockBlacklisted(block))
        {
        	return;
        }
        ...
        if(block.hasTileEntity(this.world.getBlockState(pos)))
        {
            ...
            if(TorcherinaRegistry.isTileBlacklisted(tile.getClass())) return;
            for(int i = 0; i < this.speed(this.speed); i++)
            {
                if(tile.isInvalid())
                {
    ...
    public void changeMode(boolean modifier)
    {
        if(modifier)
        {
            if(this.speed < TileTorcherina.SPEEDS)
            {
                this.speed++;
            }
            else
    ...
    {
    	return new TextComponentString(TileTorcherina.MODES[this.mode] + " | Speed: " + this.speed(this.speed) * 100 + "%");
    }
    
    public String getMode()
    {
    	return TileTorcherina.MODES[this.mode];
    }
